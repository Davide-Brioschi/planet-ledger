...
function addViewerEvents(){
	...
	viewer.camera.moveEnd.addEventListener(function(e) {
		...
		if(height){
			...
			altitude = height_value > 1 ? parseInt(height_value): height_value.toFixed(2);
						...
						result.forEach(item=>{
							newTempImageryLayers[item.parcel_id] = new Cesium.ImageryLayer(
								new Cesium.SingleTileImageryProvider({
									url: site_resource.uploadUrl + `/moon/${item.parcel_id}.png?${Date.now()}`,
				...
				if(result.parcels.length>0){
					...
					if(selectedEntity.id === parcelInfo.parcel_id){
						...
						tempImageryLayers[selectedEntity.id] = new Cesium.ImageryLayer(
							new Cesium.SingleTileImageryProvider({
								url: site_resource.uploadUrl + `/moon/${selectedEntity.id}.png?${Date.now()}`,
...
function getParcels(zoom){
	...
	n = Math.pow(2, zoom+2)*Math.pow(2, zoom+2);
