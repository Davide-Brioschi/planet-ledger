...
CustomGeocoder.prototype.geocode = function (input) {
	var resource = new Cesium.Resource({
	  ...
	  queryParameters: {
		'action': 'search_mars_hexagons_parcels',
		'search_input': input
	  }
	});
...
function addViewerEvents(){
		...
		if(height){
			...
			if(zoom_level < PARCELZOOMLEVEL){
                ...
                if(searchRectangle){
					...
					for(var key in tempEntities){
						var entityRectangle = tempEntities[key].rectangle.coordinates;
						if(Cesium.Rectangle.contains(entityRectangle, searchCenterCartographic)){
				...
				formData.append("action", "get_mars_hexagons_parcel_ids");
				...
				.then(result => {
					if(result.length > 0){
						result.forEach(item=>{
							...
							if(!entity.polygon.material.image){
								var imageUrl = site_resource.uploadUrl + `/mars-hexagons/${item.parcel_id}.png?${Date.now()}`;
	...
	infoBoxDescriptionElement.addEventListener('click', async function(e){
		...
		if(e.target && e.target.id== 'release-yes'){
			const formData = new FormData();
			formData.append("action", "remove_mars_hexagons_parcel");
		...
		if(e.target && e.target.id== 'update-yes'){
			...
			const formData = new FormData();
			formData.append("action", "update_mars_hexagons_parcel");
			formData.append("parcel_id", selectedEntity.id);
			...
			.then(result => {
				if(result.parcels.length>0){
					...
					selectedEntity.polygon.material = new Cesium.ImageMaterialProperty({
						image: site_resource.uploadUrl + `/mars-hexagons/${selectedEntity.id}.png?${Date.now()}`,
						color: Cesium.Color.WHITE.withAlpha(0.01),
						transparent: true
					});
					if(viewModel.show){
		...
		if(e.target && e.target.id== 'buy-yes'){
			...
			const formData = new FormData();
			formData.append("action", "add_mars_hexagons_parcel");
			formData.append("parcel_id", selectedEntity.id);
			var newImageFile = await compress(docId("parcel_image").files[0], selectedEntity.id+'.png');
			formData.append("parcel_image", newImageFile);
			formData.append("parcel_name", docId("parcel_name").value);
 ...
 function setSelection(entity){
	...
	const formData = new FormData();
	formData.append("action", "get_mars_hexagons_parcel");
	formData.append("parcel_id", selectedEntity.id);
	fetch(site_resource.ajaxUrl, {
		method: 'POST',
		body: formData
