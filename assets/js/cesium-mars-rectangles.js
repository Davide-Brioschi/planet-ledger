...
CustomGeocoder.prototype.geocode = function (input) {
	var resource = new Cesium.Resource({
	  ...
	  queryParameters: {
		'action': 'search_mars_rectangles_parcels',
		'search_input': input
	  }
	});
...
function addViewerEvents(){
		...
		if(height){
			...
			if(zoom_level < PARCELZOOMLEVEL){
				...
				parcels.forEach(entity=>{
					...
					/*
					var x_overlap = Math.max(0, Math.min(rectView.east, entityRectangle.east) - Math.max(rectView.west, entityRectangle.west));
					var y_overlap = Math.max(0, Math.min(rectView.north, entityRectangle.north) - Math.max(rectView.south, entityRectangle.south));
				...
				formData.append("action", "get_mars_rectangles_parcel_ids");
						...
						result.forEach(item=>{
							newTempImageryLayers[item.parcel_id] = new Cesium.ImageryLayer(
								new Cesium.SingleTileImageryProvider({
									url: site_resource.uploadUrl + `/mars-rectangles/${item.parcel_id}.png?${Date.now()}`,
	...
	infoBoxDescriptionElement.addEventListener('click', async function(e){
		...
		if(e.target && e.target.id== 'release-yes'){
			const formData = new FormData();
			formData.append("action", "remove_mars_rectangles_parcel");
		...
		if(e.target && e.target.id== 'update-yes'){
			...
			const formData = new FormData();
			formData.append("action", "update_mars_rectangles_parcel");
			formData.append("parcel_id", selectedEntity.id);
			...
			.then(result => {
				if(result.parcels.length>0){
					...
					tempImageryLayers[selectedEntity.id] = new Cesium.ImageryLayer(
						new Cesium.SingleTileImageryProvider({
							url: site_resource.uploadUrl + `/mars-rectangles/${selectedEntity.id}.png?${Date.now()}`,
		...
		if(e.target && e.target.id== 'buy-yes'){
			...
			const formData = new FormData();
			formData.append("action", "add_mars_rectangles_parcel");
			formData.append("parcel_id", selectedEntity.id);
			var newImageFile = await compress(docId("parcel_image").files[0], selectedEntity.id+'.png');
			formData.append("parcel_image", newImageFile);
			formData.append("parcel_name", docId("parcel_name").value);
...
function get_surface_area(lng1, lng2, lat1, lat2){
 ...
 function setSelection(entity){
	...
	toolTipElement.innerHTML= [
		...
		`<tr><th>Surface Area</th><td>${numeral(area_size).format('0,0.000000')} km<sup>2</sup></td></tr>`,
		'</tbody></table>'
	].join('');
	...
	const formData = new FormData();
	formData.append("action", "get_mars_rectangles_parcel");
	formData.append("parcel_id", selectedEntity.id);
	fetch(site_resource.ajaxUrl, {
		method: 'POST',
		body: formData
