...
function createListings() {
	...
	formData.append("action", "get_moon_layers");
	fetch(site_resource.ajaxUrl, {
		...
		result.layers.forEach(item => {
			...
			viewer.entities.add({
				...
				position: Cesium.Cartesian3.fromDegrees(lng, lat, 100, MOONIAU2000),
				billboard: {
...
function addViewerEvents(){
	...
	viewer.geocoder.viewModel.destinationFound = function(viewModel, destination) {
		...
		viewer.camera.setView({
			destination: Cesium.Cartesian3.fromRadians(center.longitude, center.latitude, 20000, MOONIAU2000)
		});
	};
	...
	viewer.camera.moveEnd.addEventListener(function(e) {
		...
		if(height){
			...
			if(zoom_level < PARCELZOOMLEVEL){
				...
				const formData = new FormData();
				formData.append("action", "get_moon_parcel_ids");
						...
						result.forEach(item=>{
							newTempImageryLayers[item.parcel_id] = new Cesium.ImageryLayer(
								new Cesium.SingleTileImageryProvider({
									url: site_resource.uploadUrl + `/moon_parcels/${item.parcel_id}.jpg?${Date.now()}`,
	...
	infoBoxDescriptionElement.addEventListener('click', async function(e){
		...
		if(e.target && e.target.id== 'buy-parcel'){
			infoBoxDescriptionElement.innerHTML = [
				...
				`<div style="margin-top:20px;text-align:center">`,
					`<button id="buy-parcel-yes" class="cesium-button" style="margin-right:20px">Submit</button>`,
		...
		if(e.target && e.target.id== 'release-parcel-yes'){
			const formData = new FormData();
			formData.append("action", "remove_moon_parcel");
		...
		if(e.target && e.target.id== 'update-parcel-submit'){
			...
			const formData = new FormData();
			formData.append("action", "update_moon_parcel");
			formData.append("parcel_id", selectedParcelEntity.id);
			...
			.then(result => {
				if(result.parcels.length>0){
					...
					tempImageryLayers[selectedParcelEntity.id] = new Cesium.ImageryLayer(
						new Cesium.SingleTileImageryProvider({
							url: site_resource.uploadUrl + `/moon_parcels/${selectedParcelEntity.id}.jpg?${Date.now()}`,
		...
		if(e.target && e.target.id== 'buy-parcel-yes'){
			// check validate 
			var checkValidity = parcelSubmitable('buy');
	        if(!checkValidity) return;
			const formData = new FormData();
			formData.append("action", "add_moon_parcel");
			formData.append("parcel_id", selectedParcelEntity.id);
			var newImageFile = await compress(selectedParcelEntity.id+'.jpg');
			formData.append("parcel_image", newImageFile);
			formData.append("parcel_name", docId("parcel-name").value);
		...
		if (e.target && e.target.id == 'add-layer-submit') {
			...
			formData.append("action", "add_moon_layer");
			...
			fetch(site_resource.ajaxUrl, {
				...
				if (result.result == 'success') {
					...
					viewer.entities.add({
						...
						name: "Landing Site",
						position: Cesium.Cartesian3.fromDegrees(longitude, latitude, 100, MOONIAU2000),
						billboard: {
							image: site_resource.baseUrl + '/assets/images/flag.png',
							width: 28,
							height: 28,
		...
		if (e.target && e.target.id == 'remove-layer-yes') {
			...
			const formData = new FormData();
			formData.append("action", "remove_moon_layer");
		...
		if (e.target && e.target.id == 'update-layer') {
			...
			infoBoxDescriptionElement.innerHTML = [
				...
				`<div class="image-box"><img id="layer-image-preview" src="${site_resource.uploadUrl}/moon_layers/${layer.image}" /></div>`,
				`<div class="input-title">Description</div>`,
		...
		if (e.target && e.target.id == 'update-layer-submit') {
			...
			var layer_title = docId("layer-title").value;
			var layer_extent = docId("layer-extent").value;
			formData.append("action", "update_moon_layer");
			formData.append("id", layer.id);
			formData.append("title", docId("layer-title").value);
			...
			fetch(site_resource.ajaxUrl, {
				...
				if (result.result == 'success') {
					...
					if(selectedMarkerEntity){
						selectedMarkerEntity.position = Cesium.Cartesian3.fromDegrees(longitude, latitude, 100, MOONIAU2000);
		...
		if (e.target && e.target.id == 'add-landmark-submit') {
			...
			formData.append("action", "add_moon_landmark");
		...
		if (e.target && e.target.id == 'remove-landmark-yes') {
			...
			const formData = new FormData();
			formData.append("action", "remove_moon_landmark");
		...
		if (e.target && e.target.id == 'update-landmark') {
			...
			for(let i=0;i<images.length;i++){
				html_arr = html_arr.concat([
					`<div class="image-box">`,
						`<img src="${site_resource.uploadUrl}/moon_landmarks/${images[i]}" />`,
					`</div>`,
		...
		if (e.target && e.target.id == 'update-landmark-submit') {
			...
			formData.append("action", "update_moon_landmark");
			formData.append("id", landmark.id);
			formData.append("title", landmark_title);
...
function get_surface_area(lng1, lng2, lat1, lat2){
 ...
 function selectLandingSite(entity){
	...
	const formData = new FormData();
	formData.append("action", "get_moon_layer");
	...
	fetch(site_resource.ajaxUrl, {
		...
		for(let i=0;i<tiles.length;i++){
			var imageryProvider = new Cesium.WebMapTileServiceImageryProvider({
				...
				layer : tiles[i],
				format : 'image/png',
				tilingScheme: new Cesium.GeographicTilingScheme({ellipsoid: MOONIAU2000, rectangle: Cesium.Rectangle.fromDegrees(extent[0],extent[1],extent[2],extent[3])}),
				ellipsoid: MOONIAU2000,
				//maximumLevel: 9,
				style : 'default',
...
function selectLandmark(entity) {
	...
	const formData = new FormData();
	formData.append("action", "get_moon_landmark");
...
function selectParcel(entity) {
	...
	infoBoxDescriptionElement.innerHTML = 'Loading <div class="cesium-infoBox-loading"></div>';
	const formData = new FormData();
	formData.append("action", "get_moon_parcel");
	formData.append("parcel_id", selectedParcelEntity.id);
	fetch(site_resource.ajaxUrl, {
		method: 'POST',
		body: formData
	...
	if(response.parcels.length > 0){
		...
		*/
		...
		if(response.client_user_id > 0){
			...
			return [
         		`<div style="margin-bottom:10px">This parcel is available.</div>`,
				`<div style="margin-bottom:10px">Please login login to buy this parcel.<div>`,
				`<div>${Math.abs(lat)}&#176${lat>0?'N':'S'} ${Math.abs(lng)}&#176${lng>0?'E':'W'}</div>`
			].join("");
		}
	}
...
docId("list-layers").addEventListener("click", function (e) {
	if (e.target && e.target.nodeName === "LI") {
		...
		var destination = Cesium.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, cartographic.height+5000000, MOONIAU2000);
		viewer.camera.flyTo({
...
docId("list-landmarks").addEventListener("click", function (e) {
	if (e.target && e.target.nodeName === "LI") {
		...
		if (entity) {
		    var cartographic = Cesium.Cartographic.fromCartesian(entity.position.getValue()); //WGS84
		    var destination = Cesium.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, 20000, MOONIAU2000);
			viewer.camera.setView({
				destination: destination
			});
...
function layerInfoDescription(response) {
	...
	var html_arr = [
		`<div class="title">${layer.title}</div>`,
   	    `<div class="image-box">`,
		    `<img src="${site_resource.uploadUrl}/moon_layers/${layer.image}" />`,
		`</div>`,
...
function landmarkInfoDescription(response) {
	...
	for(let i=0;i<images.length;i++){
		html_arr = html_arr.concat([
			`<a class="image-box" href="${site_resource.uploadUrl}/moon_landmarks/${images[i]}" target="_blank">`,
				`<img src="${site_resource.uploadUrl}/moon_landmarks/${images[i]}" />`,
